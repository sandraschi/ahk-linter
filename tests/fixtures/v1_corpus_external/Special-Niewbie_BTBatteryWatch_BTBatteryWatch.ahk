...
GetBatteryIconFile(level, themePath) {
    ...
    ; Check for battery default 20%, customizable via LBTMessageSet
    ...
    if (level <= lowBatteryThreshold) {
        ...
        if (!LowBatteryWarningShown && batteryWarningMessageToggleState = "1") {
            ...
            ; Imposta un timer per resettare l'avviso dopo un certo periodo di tempo
            SetTimer, ResetLowBatteryWarning, -1800000 ; 30 minuti
        }
    }
...
GetStandardBatteryIcon(level, themePath) {
    level := Round(level)
	; Check if the battery default 20%, customizable via LBTMessageSet
    lowBatteryThreshold := GetLowBatteryThreshold()
    if (level <= lowBatteryThreshold) {
        ; Variabile globale per evitare messaggi ripetuti
        global LowBatteryWarningShown, batteryWarningMessageToggleState
        if (!LowBatteryWarningShown && batteryWarningMessageToggleState = "1") {
            LowBatteryWarningShown := true
            ShowLowBatteryWarning(level)
            ; Imposta un timer per resettare l'avviso dopo un po' di tempo
            SetTimer, ResetLowBatteryWarning, -1800000  ; 30 minuti
        }
    }
