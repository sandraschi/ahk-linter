...
^!b::  ; Ctrl+Alt+B for bugs
    ...
    ; Create bug GUI
    Gui, BugGuibugCount:New, +AlwaysOnTop -Caption +ToolWindow +E0x20
    Gui, Color, 000000
    ...
    Gui, Show, xx yy w50 h50, BugbugCount
    WinSet, Transparent, 200, BugbugCount
    
    ; Start bug movement
    SetTimer(MoveBugbugCount, 50)
    
    ; Store bug data
    bugbugCount_x := x
    bugbugCount_y := y
    bugbugCount_xSpeed := xSpeed
    bugbugCount_ySpeed := ySpeed
...
MoveBug10:
    ...
    ; Update position
    bugbugNum_x += bugbugNum_xSpeed
    bugbugNum_y += bugbugNum_ySpeed
    
    ; Bounce off edges
    if (bugbugNum_x <= 0 || bugbugNum_x >= A_ScreenWidth - 50)
        bugbugNum_xSpeed *= -1
    if (bugbugNum_y <= 0 || bugbugNum_y >= A_ScreenHeight - 50)
        bugbugNum_ySpeed *= -1
    
    ; Move window
    WinMove, BugbugNum,, % bugbugNum_x, % bugbugNum_y
    
    ; Random direction changes
    rand := Random(), 1, 100
    if (rand = 1) {
        bug := Random()bugNum_xSpeed, -5, 5
        bug := Random()bugNum_ySpeed, -5, 5
    }
    return
...
^!f::  ; Ctrl+Alt+F for funny BSOD
    ...
    rand := Random(), 1, % funnyMessages.MaxIndex()
...
ElizaSend:
    ...
    GuiControl,, ChatDisplay, chatYou: UserInput`n
    Gosub, ElizaRespond
    return
...
ElizaRespond:
    ...
    for pattern, responseArray in responses {
        if (InStr(inputLower, pattern)) {
            ...
            if (InStr(responseArray[1], "{0}")) {
                ...
                rand := Random(), 1, % responseArray.MaxIndex()
                response := responseArray[rand]
            }
            
            GuiControlGet, chat,, ChatDisplay
            GuiControl,, ChatDisplay, chatELIZA: response`n`n
            responseFound := true
            break
        }
    }
    ...
    if (!responseFound) {
        rand := Random(), 1, % defaultResponses.MaxIndex()
...
NewGame:
    ...
    loop 9 {
        row := A_Index
        loop 9 {
            GuiControl,, Cellrowcol, 
            GuiControl, +BackgroundWhite, Cellrowcol
        }
    }
    ...
    for _, cell in startingCells {
        GuiControl, +ReadOnly, Cellcell
        GuiControl, +BackgroundE6E6E6, Cellcell
    }
    return
...
^!x::  ; Ctrl+Alt+X to clean up all bugs
    loop 10 {
        Gui, BugGuiA_Index:Destroy
        SetTimer, MoveBugA_Index, Off
    }
    bugCount := 0
    return
