...
uv sync --group dev
uv run ruff check ahk_lint.py checks fixers config.py mcp_server.py lsp_client.py
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
uv run ruff format --check ahk_lint.py checks fixers config.py mcp_server.py lsp_client.py
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
