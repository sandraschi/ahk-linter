...
\begin{abstract}
AutoHotkey (AHK) is a widely-used Windows automation language with millions of installations. Its 2022 v2 release broke compatibility with thousands of existing scripts, yet no academic work exists on AHK v2 tooling. We present \texttt{ahk-lint}, the first linter for AutoHotkey v2, built using agentic development combining static analysis with AI-assisted repair. The linter detects 167 known v1-to-v2 migration patterns across 9 categories, provides auto-fix for 7/10 primary patterns, and integrates with the Model Context Protocol (MCP) for agent self-checking. Evaluation on 7 legacy v1 scripts shows 343 detected issues (191 errors). An LLM generation test reveals that AI models routinely produce v1 syntax---5 generated scripts contained 32 issues (25 errors). Coverage analysis against the community-maintained mmikeww converter shows 51\% of the ~325 known patterns are implemented, with the remaining requiring procedural transformation logic. On the fleet's 80+ production v2 scripts, the linter reports zero false positives.
\end{abstract}
...
\begin{enumerate}
  \item \texttt{ahk-lint}, the first linter for AutoHotkey v2, with 167 patterns across 9 categories, auto-fix, config, and SARIF output.
...
This work was developed using AI-assisted coding tools including Claude Opus 4.8 (Anthropic) and DeepSeek V4 Flash. The rule set was derived from the community-maintained \texttt{AHK-v2-script-converter} by \texttt{mmikeww}. The authors thank the AutoHotkey community for two decades of contributions that made this work possible.
